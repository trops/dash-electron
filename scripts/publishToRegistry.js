 ...
 * - A GitHub release with a ZIP asset (run `npm run package-zip` + create release first)
 *
 * Usage:
...
function main() {
    ...
    const downloadUrl = repoUrl
        ? `${repoUrl}/releases/download/v{version}/${packageName}-v{version}.zip`
        : "";
    ...
    console.log(`\nWidgets: ${widgets.map((w) => w.name).join(", ")}`);
    ...
    const manifestPath = `packages/${packageName}/manifest.json`;
    ...
    const createResult = exec(
        ...
        -X PUT \
        -f message="Add ${packageName} v${version}" \
    ...
    if (!createResult || createResult.includes("error")) {
        ...
        if (existingSha && !existingSha.includes("Not Found")) {
            exec(
                `gh api repos/${ghUser}/dash-registry/contents/${manifestPath} \
                -X PUT \
                -f message="Update ${packageName} to v${version}" \
                -f content="${encodedContent}" \
                -f branch="${branchName}" \
...
**Author:** ${author}
**Version:** ${version}
**Widgets:** ${widgets.map((w) => w.displayName || w.name).join(", ")}
